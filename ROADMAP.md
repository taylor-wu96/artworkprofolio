...
- **🎨 藝術／設計目標** — 這件事讓觀者**感覺**到什麼？它服務「潮濕的質地」的哪一面？
...
> 落地備註：`序列 ／ 總覽` 切換、印樣台密排（齒孔編號）、當前格綠括弧（暗房）／實墨（亮房）、點格顯影跳入、鍵盤方向鍵移格＋Esc 退回序列、觸控原生捲動、縮圖複用已載入來源（不重打高解析）。已於 8 圖影像集（Shueinandong）桌機＋手機驗證，build 18 頁通過、無 console error。**修補（2026-06-27）**：短視窗（IG 內建瀏覽器）切換帶與印樣格重疊——頂部 padding 由 `9vh`（會隨視窗塌陷）改固定 min `clamp(4.7rem,7vh,5.6rem)` 恆清開切換帶；加頂部遮罩漸層帶（捲動縮圖沒入控制下而非穿幫）；`min-height:0` 保長影像集內部捲動到底。340/375/414/452 多尺寸驗證 clearance +12～16px。**再修（2026-06-27）**：印樣台原以 `aspect-ratio:3/2 + object-fit:cover` 強裁，混合直橫幅影像集裡直幅被裁成橫條（違背 DESIGN §5 永不裁切）。改為 **masonry**：外層垂直捲動、內層多欄（`column-width`），每格帶原生寬高、`height:auto`，各圖以**原生比例完整呈現不裁切**。驗證直幅格比例 0.67／橫幅 1.5 皆吻合原圖、3 欄滿版、無橫向溢出、當前格綠框與切換帶 clearance 皆保留。
...
#### 🎨 藝術／設計目標
分享卡是**這座美術館伸到牆外的一隻手**。它必須帶著美術館的氣質出門：暗場、封面影像、標題、那條機器簽名。一張對的 OG 圖，是讓「潮濕的質地」在別人的 timeline 裡也成立——不是 SEO 技術債，是**展覽的延伸立面**。
...
> **▸ 後續**：v1 的 H0／H1／H2.1 已全數落地（儀器齊全）。下一輪迭代——讓零件連成「一個連續的身體」、並建立護欄使迭代可控——見 [ROADMAP-v2.md](ROADMAP-v2.md)。
