...
## 8. 綠色用量（v2.2 調升為 tier 2）

v2 把綠壓得太克制；v2.2 調升：綠是**生機與「可被觸碰」的訊號色**，
出現在所有「狀態 + 互動」處——但**仍不進背景/大色塊**，保留灰階主調。

| ✅ 准用（tier 2） | ❌ 仍不准 |
|---|---|
| `aria-current` 當前所在 | 任何背景／大色塊填色 |
| `focus-visible` 聚焦環 | 按鈕底色 |
| 閱讀進度條填充 | 大面積 icon／裝飾色塊 |
| 檔案索引「最新／進行中」標記 | 正文大段文字本身 |
| **內文連結下緣細線（常態）＋ hover 整體轉綠** | |
| **互動 hover：nav 下緣、圖錄條目標題、meta 連結、聯絡/主題連結** | |
| 3D 光場中的綠光（v2.2 增量：桌機 44／行動 24 點） | |

用量手感：綠是「被啟動的東西」——靜止時是少數信號，**滑鼠所到之處生機亮起**。
...
**侵蝕曲面（已落地・P4）**：光雲之後獨立疊一層**傾斜的細線網格地面**，頂點噪聲緩慢位移＝
衰敗，灰階極淡、邊緣沒入霧、隨滾動消融。「衰敗」由此元素承載，光雲（生機/沉思）原貌完全保留
（見 PROGRESS B5-2）。綠仍只在那一粒活光點——衰敗的地面是灰的，活著的是綠的。
...
**2026-06-23 v3（藝術融合・觀看的機器）**
- 標題尺度收斂（display max 4.6→3rem、hero 3.4→2.7rem、h2 1.4→1.2、essay 標題下修）。
- 綠調升至 tier 2（見 §8）：內文連結、互動 hover、3D 綠光增量。
- 新增「觀看的機器」四層融合（見 §13）：資料精度層（Ikeda）、機器視覺準星（凝視）、綠霧（Janssens）。

**2026-06-25 v4（成熟化・響應式與觸控）**
- 響應式正式化：**直幅為閱讀／索引主軸；橫幅手機切換成影像優先**（chrome 收最小）。斷點收斂為四個具名值（見 §14），不再散寫頁級斷點。
- 觸控命中區：coarse pointer 一律 ≥44px（透明 padding 擴張，不改視覺尺寸）；hover-only affordance 須有觸控等價（§14）。
- 影像放映誤觸（修真實缺陷）：coarse pointer **取消「點暗場關閉」**，關閉只經明確 close 鈕＋向下拖曳；tap/swipe/zoom 以位移＋時間消歧（見 §15）。
- 房間一致：**亮房影像不喚暗房放映**——閱讀不被監視（呼應 §13 準星只在暗房）。
- 可維護性立場：@layer 六層地基（PROGRESS 階段 I）已足夠收斂，**系統不是太大而是夠用**；真正風險＝頁級 `<style>` 漂移，共用模式應下沉 components 層（見 §14.4）。本站規模不引入 CSS-in-JS／token 產生器／元件庫。

---

## 13. 觀看的機器（v3 藝術融合）

把攝影 zine 重構成「**一台觀看光的精密儀器**」。四類當代藝術脈絡各佔一層，
由「綠＝活著的訊號」串起，不互相打架：

| 層 | 來源 | 化身 | 程式 |
|---|---|---|---|
| **表面 skin** | Ikeda・Anadol・Reas | 等寬字**資料條**：每件作品決定性的座標／編碼／頻道，綠＝資料訊號 | `lib/signature.js`、`.data-strip` |
| **氛圍 depth** | Turrell・Eliasson・Janssens | 3D **綠霧**＋增量綠光（彩色感知霧） | `Hero3D.jsx` haze plane |
| **互動 behavior** | Steyerl・Lozano-Hemmer・Paglen | **機器視覺準星**跟隨游標，hover 作品時框選鎖定（凝視／監視） | `Base.astro` reticle script、`.reticle` |
| **手勢 signature** | JODI・Rozendaal | 會反應的準星＋站如儀器，即 net-art 反應手勢 | 同上 |

**資料條（signature）**：`lib/signature.js` 用 FNV-1a + LCG 由 slug 種子產生**決定性**
的 `±緯度° ±經度°`、`0x` 編碼、`CH.頻道`。不是真 EXIF，而是 Ikeda 式「資料即美學」——
每件作品有一組屬於自己、可重現的識別碼。等寬字（`--font-mono`），`__sig` 一欄用綠。

**機器視覺準星（reticle）**：暗房（圖錄）專屬。綠色準星跟隨游標（rAF lerp 微滯），
hover 作品（`.entry__plate / .work__cover / .gallery-item__media / .plates figure`）時
四角括弧**框選鎖定**該影像、隨捲動追蹤。`pointer: coarse`（觸控）與 `prefers-reduced-motion`
不啟用；隨房間切換建立／清理。亮房（閱讀）不出現——閱讀不被凝視。

**綠霧（haze）**：3D 光場後墊一層極淡、極慢呼吸的綠色 additive plane（Janssens 彩色霧），
隨捲動淡出；綠光點同步增量（桌機 44／行動 24）。
...
v1–v3 的響應式是「能用」：12→8→4 欄、手機導覽缺陷已修（階段 I）。v4 把它**正式化**，
並補上兩個一直缺席的維度：**螢幕方向（orientation）** 與 **觸控命中區**。

### 14.1 具名斷點（收斂散值）

程式裡散落 520／560／720／1180／1440 等手寫斷點，語意重疊、難維護。收斂為四個**具名規約值**
（CSS 變數無法用於 `@media`，故此處是團隊約定，不是 live token——新斷點一律取這四個）：

| 名稱 | 值 | 分界意義 |
|---|---|---|
| `sm` | **560px** | 手機 → 大手機／窄平板；header 由兩列收回單列、上下篇由單欄轉兩半 |
| `md` | **720px** | 單欄 → 雙欄；側欄 meta 開始 `sticky`、12 欄網格生效 |
| `lg` | **1180px** | 旁註（sidenote）由 inline 浮出 measure 右側頁邊 |
| `xl` | **1440px** | 內容最大寬；超過只增外緣留白 |

> 遷移原則：既有 520/720/768/1180 逐步對齊到上表；**不為單一頁面新增第五個斷點**。

### 14.2 螢幕方向（直幅 vs 橫幅）

| 方向 | 情境 | 表演原則 |
|---|---|---|
| **直幅（portrait，預設）** | 滑手機讀文、逐頁翻索引 | 閱讀／序列的主軸。單欄、垂直節奏、`sticky` 側欄在 ≥md 才出現 |
| **橫幅手機（landscape ＋ 矮高度）** | 轉橫看一張照片、放映 | **影像優先**：hero／放映／封面吃滿視口，介面 chrome 收到最小（垂直空間稀缺） |

偵測橫幅手機：`@media (orientation: landscape) and (max-height: 540px)`。
此模式下：放映影像放寬 `max-height`、chrome 壓成單行；hero 縮短、長文維持單欄但加大左右留白。
**桌機橫向不套此規則**（靠 `max-height` 排除）。

### 14.3 觸控命中區與 hover 等價

- **最小命中區 44×44px**（Apple HIG）給所有 coarse-pointer 可點元素：導覽連結、聲音開關、
  放映 close／翻頁／縮放、上下篇。手法＝**透明 padding／`::before` 擴張命中區，不改視覺尺寸**
  （維持克制的視覺、給拇指足夠的靶）。
- `@media (hover: none)`：所有「hover 才顯」的資訊或操作必須有觸控等價——
  essay 列表的 `→`、contact 連結底線、related 縮圖位移、nav 下緣綠線。
  **原則：hover 是錦上添花，觸控不得因此失去資訊或入口。**
- **安全區**：固定元素（header、放映 chrome／close）加 `env(safe-area-inset-*)`，避開瀏海與圓角。
- reticle（§13）維持 `pointer: coarse` 不啟用——觸控沒有游標，準星無依附。

### 14.4 可維護性契約（系統不是太大，是要守住邊界）

@layer 六層（`tokens/base/layout/components/machine/media`，階段 I）已解決單檔肥大與 cascade
不可控。系統**夠用且收斂**；真正的維護風險是**頁級 `<style>` 漂移**——`post/[slug]` ~300 行、
`about` ~120 行的 scoped 樣式各自手寫 grid 與斷點，易與 token／斷點規約脫鉤。

契約：
1. 頁級 `<style>` 只放**該頁獨有**的版面；共用模式（`work__header`／meta 列／`section-head`／
   `post-nav`）下沉到 `components` 層。
2. 不再新增頁級一次性斷點散值；斷點一律取 §14.1 四值。
3. 間距／字級／動態一律走 token，不手寫散值（沿用 §4）。
4. **不過度抽象**：本站規模不引入 CSS-in-JS／設計 token 產生器／元件庫——token＋@layer＋
   Astro 元件已是正確顆粒度。加複雜度＝減維護性。

---

## 15. 影像觸控契約（v4・修放映誤觸）

**病根（誠實點出）**：`Lightbox.astro` 的 click 委派裡「點 `.pjx` 暗場即關閉」對**滑鼠**合理，
對**觸控**危險——三重誤觸疊加：

1. 手機影像受 `max-height: 80vh` 限，上下留出大片「一點就關」的暗場；
2. 左右 `.pjx__nav` 隱形熱區各 ~14vw，**疊在影像邊緣上**，想看圖卻翻頁；
3. 單擊影像切 2× 縮放——與「想關閉」的意圖打架。
4. 額外：about 亮房肖像 `zoomable` 會喚起**暗房**放映（房間錯置）。

**契約（coarse pointer）**：

1. **取消「點暗場關閉」**。關閉只經 ① 明確 close 鈕（≥44px、safe-area）② **向下拖曳關閉**
   （drag-to-dismiss，過閾值才觸發、跟手有位移回饋）。
2. **tap / swipe / zoom 消歧**：位移 <10px 且 <250ms ＝「tap」；tap 在 coarse 下**不做破壞性動作**。
   縮放改 **double-tap**（標準手勢）；單 tap 切換 chrome 顯隱（看純淨影像 ↔ 看資料）。
3. **翻頁**：coarse 下移除與影像重疊的左右邊緣隱形熱區，改靠 ① 水平 swipe（已具）
   ② 底部 chrome 內的**明確 prev／next 鈕**。fine pointer（滑鼠）維持邊緣熱區（懸停有箭頭提示）。
4. **房間一致**：亮房影像（about 肖像、essay 封面）**不喚暗房放映**——
   就地放大（亮場 inline lightbox），閱讀不被監視（呼應 §13）。暗房影像才進「觀看儀器」放映。
5. 命中區與安全區：close／nav／zoom 全 ≥44px；固定 chrome 加 `env(safe-area-inset-*)`。

落地見 [PROGRESS 階段 L](PROGRESS-REDESIGN.md)。
